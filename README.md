...
All content lives in `src/data.js`, including name, title, bio, experience, skills, projects,
and contact/social links. Section components live in `src/components/`.
...
- **Contact form**: submits via a `mailto:` link (opens the visitor's email client with the
  message prefilled), so no backend is required. To collect submissions without relying on
  the visitor's mail client, wire `src/components/ContactForm.jsx` to a service like
  Formspree or EmailJS instead.

## Deploy (GitHub Pages)

Push to `main` and the included GitHub Actions workflow (`.github/workflows/deploy.yml`)
